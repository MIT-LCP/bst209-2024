...
1. Explain the bias-variance tradeoff using examples of a simple model and a complex model. How do these models differ in terms of bias and variance?

2. How does regularization help manage the bias-variance tradeoff in machine learning models?

3. Provide a real-world example where balancing bias and variance is crucial for model performance.

## Solution

1. Simple models have high bias but low variance, as they cannot capture the complexity of the data. Complex models have low bias but high variance, as they fit the training data too closely, capturing noise.

2. Regularization introduces a penalty to model complexity, shrinking coefficients and reducing variance at the cost of increasing bias. This tradeoff helps achieve better generalization.

3. In almost all predictive tasks (including our mortality prediction example), overfitting will lead to unreliable performance, so balancing bias and variance is essential.
...
1. Compare ridge regression and lasso in terms of their penalty terms and effects on model coefficients. How do these techniques address overfitting?

2. How do L1 and L2 regularization relate to the distance metrics discussed in our earlier workshop?

3. In what situations might you prefer lasso over ridge regression and vice versa? Provide examples.

4. Discuss how regularization affects model interpretability and performance.

## Solution

1. Ridge regression uses the L2 norm, shrinking all coefficients but keeping them non-zero. Lasso uses the L1 norm, encouraging sparsity by setting some coefficients to zero.

2. L1 regularization corresponds to Manhattan distance (sum of absolute values). L2 regularization corresponds to Euclidean distance (sum of squared values).

3. Lasso is preferred when feature selection is important, as it results in sparser models. Ridge regression is useful when all features are potentially relevant (e.g. but multicollinearity needs to be addressed).

4. Regularization improves performance by controlling overfitting and enhances interpretability by simplifying models, particularly with lasso's variable selection.
...
In order to actually shrink our coefficients, we need to pass in one or more non-zero values to the `lambda` argument. Or, if we don't specify anything at all, `glmnet` will automatically generate a grid of 100 $\lambda$ values to try.

The key parameter to watch out for is `alpha`. If we want to fit a ridge regression model, we set `alpha = 0`. If we want lasso, we set `alpha = 1`. 
