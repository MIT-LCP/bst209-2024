...
*While we aren't do any kind of scaling of variables in this exercise, in practice you'll probably want to do this when working on any distance-based task.*
...
In this context, finding the distance is the same formula we learn at school, when we find the length of the hypotenuse using Pythagoras theory $(a^2+b^2=c^2)$.
...
We can then demonstrate how K-means clustering might identify these distinct groups:

```{r}
# Apply K-means clustering with 2 clusters
# n_start is a parameter that runs n different random initializations
...
While we explicitly stated a value for $K$ in this example, there are several methods that are used to automatically determine the number of clusters (for example, the "elbow" method, below). 

![](./images/elbow_method.jpg)

We won't explore those methods here, but you can find plenty of resources on the web if you would like to do a deeper dive.
...
1. Calculate the Euclidean and Manhattan distance between point `A(2,3)` and `B(5,7)`.

2. Is k-means a supervised or unsupervised approach?

## Solution

1. `euclidean_distance(2, 3, 5, 7)` 

   `manhattan_distance(2, 3, 5, 7)`  

2. Unsupervised (we are not providing labels to the algorithm to learn from).

## K-Nearest Neighbors

K-nearest neighbors (kNN) is another distance-based algorithm (this time supervised). With kNN, for any point for which we want to predict a class, we look for the $k$ nearest points and then take an average of these points. 

Let's use the eICU data we've been using this week and add another predictor, `heartrate`, to our models so that now we are using `apachescore` and `heartrate` to predict mortality. 
...
g1 <- p_x %>% mutate(f_hat = f_hat) %>%
  ...
  guides(fill=FALSE) +  
  stat_contour(aes(x=apachescore, y=heartrate, z=f_hat),
               data=p_x, breaks=c(0.5), color="black",lwd=1.5)
...
With $k = 7$ we obtain what looks like a reasonable estimate of the true $f$. There are still a couple of red islands in the red area, but it's probably our best effort for this particular $k$NN model.
...
1. How does the choice of $k$ in k-NN affect the model's tendency to overfit or underfit the data?

2. What is one strategy to mitigate overfitting in k-NN?

3. What resampling method could we use to select an optimal value for $k$?

## Solution

1. A smaller $k$ leads to more complex decision boundaries, increasing the risk of overfitting as the model captures noise in the training data.

2. Larger $k$ values smooth the decision boundary but may lead to underfitting by oversimplifying the data structure.

3. Cross-validation can help select an optimal $k$ by evaluating model performance on different subsets of the data.

## Curse of dimensionality

In machine learning, the "curse of dimensionality" refers to problems that arise in very high-dimensional spaces. Put simply, when we add features, our distances start to become less informative. In very high-dimensional spaces, datasets become increasingly "sparse". Sparse data, can lead to issues such as overfitting.

## Exercise

1. How might the curse of dimensionality impact the performance of distance-based algorithms like k-NN and K-Means?

2. How might you reduce dimensionality?

## Solution

1. Distance metrics become less effective in high-dimensional spaces, making it difficult for algorithms to find meaningful patterns.

2. Feature selection is one way to reduce dimensions while retaining important information, leading to more robust and interpretable models.
